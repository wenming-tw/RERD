...
class RGCN(nn.Module):
    ...
    def forward(self, data, CL=False):
        ## text graph
        if CL:
            x_t, edge_index_t, edge_type_t, batch_t = data.x_t_1, data.edge_index_t_1, data.edge_type_t_1, data.x_t_1_batch
        else:
            x_t, edge_index_t, edge_type_t, batch_t = data.x_t, data.edge_index_t, data.edge_type_t, data.x_t_batch
        ...
        x_t = scatter_mean(x_t, batch_t, dim=0)
...
class GCN_Net(nn.Module):
    ...
    def forward(self, data, CL=False):
        ...
        x_g = torch.cat([x_t, x_p], dim=1)
        x_fusion = torch.relu(self.fusion(x_g))

        logit = self.fc(x_fusion)
        logit = F.log_softmax(logit, dim=1)

        if CL:
            loss = self.gcl(data, x_fusion)
            return logit, loss
        else:  
            return logit

    def gcl(self, data, x_fusion_0):
        x_t_1, x_p_1 = self.encoder(data, CL=True)
        x_g_1 = torch.cat([x_t_1, x_p_1], dim=1)
        x_fusion_1 = torch.relu(self.fusion(x_g_1))     

        loss = self.simsiam(x_fusion_0, x_fusion_1)
        return loss.mean()
...
if __name__ == '__main__':    
    ...
    parser.add_argument('--log_dir', default='../logs/Semi/Ours/')
    parser.add_argument('--ckpt_dir', default='../ckpt/Semi/Ours/')
    ...
    for iter in range(args.iterations):
        ...
        accs0, Pre1_0, Pre2_0, Rec1_0, Rec2_0, F1_0, F2_0 = train_GCN(fold0_x_test, fold0_x_train_l, args, experiment_id, iter, 0)
        accs1, Pre1_1, Pre2_1, Rec1_1, Rec2_1, F1_1, F2_1 = train_GCN(fold1_x_test, fold1_x_train_l, args, experiment_id, iter, 1)
        accs2, Pre1_2, Pre2_2, Rec1_2, Rec2_2, F1_2, F2_2 = train_GCN(fold2_x_test, fold2_x_train_l, args, experiment_id, iter, 2)
        accs3, Pre1_3, Pre2_3, Rec1_3, Rec2_3, F1_3, F2_3 = train_GCN(fold3_x_test, fold3_x_train_l, args, experiment_id, iter, 3)
        accs4, Pre1_4, Pre2_4, Rec1_4, Rec2_4, F1_4, F2_4 = train_GCN(fold4_x_test, fold4_x_train_l, args, experiment_id, iter, 4)
