...
class RGCN(nn.Module):
    def __init__(self, in_dim_t, in_dim_p, hid_dim, out_dim):
        ...
        self.text_conv1 = GCNConv(in_dim_t, hid_dim)
        self.text_conv2 = GCNConv(hid_dim, out_dim)
        self.text_bn1 = nn.BatchNorm1d(hid_dim)
        self.text_bn2 = nn.BatchNorm1d(out_dim)
        
        self.prop_conv1 = GCNConv(in_dim_p, hid_dim)
        self.prop_conv2 = GCNConv(hid_dim, out_dim)        
    ...
    def forward(self, data, CL=False):
        ...
        x_t = self.text_conv1(x_t, edge_index_t)
        x_t = self.relu(self.text_bn1(x_t))

        x_t = self.text_conv2(x_t, edge_index_t)
        ...
        x_p = self.prop_conv1(x_p, edge_index_p)
        x_p = self.relu(self.prop_bn1(x_p))

        x_p = self.prop_conv2(x_p, edge_index_p)
...
if __name__ == '__main__':    
    ...
    parser.add_argument('--log_dir', default='../logs/Ours_gcn/')
    parser.add_argument('--ckpt_dir', default='../ckpt/Ours_gcn/')
